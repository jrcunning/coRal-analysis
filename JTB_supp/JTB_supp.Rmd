...
* $\sigma_{NH}, \sigma_{NS}$: Proportions of nitrogen recycled from host and symbiont biomass turnover
    ...
    + Values used: `r with(defpars, c(sigmaNH, sigmaNS))` (dimensionless)
* $\sigma_{CS}$: Proportion of metabolic CO~2~ produced by symbiont recycled passively to photosynthesis
    ...
    + Value used: 0.9 (dimensionless)
* $\sigma_{CH}$: Proportions of metabolic CO~2~ produced by host recycled passively to photosynthesis 
...
# Figure S9. Hysteresis under different nutrient and feeding regimes.
```{r runFigS9, cache=T, include=F}
# Set up cluster for parallel processing
cl <- makeCluster(detectCores())  # Initiate cluster
registerDoParallel(cl)

# Set input values for steady state runs
input <- expand.grid(L=seq(10,55,1), initS=c(0.0001,0.1))
...
```{r plotFigS9, cache=T, fig.width=7, fig.height=7, echo=F, message=F}
# Create plot
par(mfrow=c(2,2), mar=c(3,3,1,1))

plot(NA, xlim=range(input$L), ylim=c(0,0.3), xlab="Light", ylab="Steady state S:H ratio")
title("DIN=1e-7 mol/L; X=2e-6 mol/L")
points(c(48,48,53,53), c(0.22,0.23,0.22,0.23), pch=c(19,1,19,1), cex=c(0.4,1,0.4,1), col=c("black","black","red","red"), xpd=T)
text(38, 0.225, labels="init. S:H", xpd=T, cex=0.7, srt=90)
text(46, c(0.22,0.23), labels=c("0.1", "0.0001"), xpd=T, pos=2, cex=0.7)
text(50, 0.25, labels=c("Steady state growth"), xpd=T, cex=0.7)
text(c(48,53), 0.24, labels=c("pos.", "neg."), xpd=T, cex=0.7)
apply(res1, 1, FUN=function(x) {
  ...
  with(x[x$sh>0.3, ], {
    points(L, rep(0.3, length(L)), pch="^", col="red")
    points(L, rep(0.29, length(L)), pch=1, col="red", cex=1)
  })
})
...
apply(res2, 1, FUN=function(x) {
  with(x, points(L, sh,
                 ...
                 cex=ifelse(initS==1e-01, 0.4, 1)))
  with(x, if (sh > 0.3) {
    points(L, 0.3, pch="^", col="red")
    points(L, 0.29, pch=1, col="red", cex=1)
  })
})
...
These figures show steady state S:H biomass ratios in constant environments with varying DIN and food (X) levels, with initial S:H biomass values of either 0.1 or 0.0001. Points are colored according to whether the host exhibits positive (black) or negative (red) growth at steady state.
