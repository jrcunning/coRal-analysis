...
93106, USA}
  ...
  dynamic responses of bleaching and recovery, and suggests that these
  phenomena represent transitions between alternative stable states. A
  suite of complex observed behaviors reproduced by the model suggests it
  captures many important attributes of the system; meanwhile, its modular
  framework and open source R code are designed to facilitate expansion
  and problem-specific applications. We see high utility for this modeling
  framework in generating testable hypotheses and predicting integrated,
  mechanistic responses of corals to environmental change.
  
  Latest development--sigmaCH and sigmaCS changed to 0.1 meaning 10\% of
  metabolic CO2 produced is passively available to photosynthesis, to
  reflect the fact that that \textasciitilde{}10\% of total internal DIC
  at pH 7 is in the form of CO2. probably actually less than this since
  internal pH may be closer to 7.5. This does not change any of the
  figures substantively except the hysteresis figure--overshoot no longer
  occurs with specific parameters run. may need to just change light range
  or rate of change to reproduce overshoot.
  \end{abstract}
...
generated by host biomass turnover is passively made available to the
