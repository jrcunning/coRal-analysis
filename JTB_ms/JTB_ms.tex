...
parameters (except varying nNX), and DIN=1e-7 molN/L, prey=1e-6 CmolX/L,
and initial symbiont biomass=0.0001.}
\end{figure}
...
nitrogen-limitation, such as low external DIN, high feeding rates, will
prevent an overshoot from occurring at all, since nitrogen-limitation is
re-established in the system before S:H biomass ratios reach high
levels. While many scenarios are possible under different environmental
...
that vary the N:C ratio of host's heterotrophic food source (Fig. 8):
...
Importantly, the re-establishment of nitrogen-limitation that
characterizes the `healthy' symbiosis is the most important diagnostic
for recovery, as this is when high carbon fixation and growth rates are
resumed. A high S:H biomass ratio alone does not indicate a `healthy'
state has been reached, because overall C-limitation could still persist
and thus the host isn't really growing. This could explain why corals
that have apparently recovered their symbiont populations after
bleaching still do not grow or suffer reduced reproduction until much
later (xxx). Together, these dynamics indicate that acquisition of
carbon from a source other than the symbiont is extremely important for
the host in order to recover from bleaching. The system can `recover'
(in terms of resuming positive growth and N-limitation) more rapidly
when the coral obtains more carbon. Indeed, feeding has been shown to
promote more rapid recovery from bleaching (xxx). Moreover, if the host
could obtain an independent source of carbon, such as direct uptake of
DOC (xxx), this could be even more important in ensuring rapid recovery
from a bleached state.
