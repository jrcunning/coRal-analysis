...
run_coral_jCO2a <- function(time, env, pars, NPQ="inhibited") {
  require(dplyr)
  ...
  jCO2 <- seq(jCO2a_vals[1], jCO2a_vals[2], along=time)
  # Set initial values
  # ==================
  # Host
  H <- data_frame(
  ...
  for (t in 2:length(time)) {
    ...
    H$jCO2a[t] <- S$jCP[t-1] * S$S[t-1]/H$H[t-1] * 0.01  # active delivery of CO2 from the environment (mediated by the host)
...
run_coral_jCO2a_manual <- function(time, env, pars, NPQ="inhibited", jCO2a_vals=c(0.3,0)) {
  require(dplyr)
  # Define parallel complementary synthesizing unit function
  synth <- function(x, y, m) 1/((1/m)+(1/x)+(1/y)-(1/(x+y)))
  synth <- function(x, y, m) (m^-2 + x^-2 + y^-2)^(-1/2)
  ...
  for (t in 2:length(time)) {
    ...
    # CO2 input flux
    H$jCO2p[t] <- pars$jCO2p # passive CO2 flux from the environment
    H$jCO2a[t] <- jCO2[t]  # active delivery of CO2 from the environment (mediated by the host)
    H$jCO2[t] <- jCO2[t]  # Total CO2 input from the environment
    S$rCS[t] <- pars$jST0 * pars$sigmaCS  # metabolic CO2 recycled from symbiont biomass turnover
    H$rCH[t] <- H$jHT[t-1] * pars$sigmaCH  # metabolic CO2 recycled from host biomass turnover
    # Production flux (photosynthetic carbon fixation)  # NOT MULTIPLYING jCO2 BY H/S when MANUAL INPUT
    S$jCP[t] <- synth(S$jL[t] * pars$nLC, H$jCO2[t] + H$rCH[t]*H$H[t-1]/S$S[t-1] + S$rCS[t], pars$jCPm) / S$cROS[t-1]
    # Rejection flux: CO2 (wasted to the environment)
    S$jCO2w[t] <- max((H$jCO2[t] + H$rCH[t])*H$H[t-1]/S$S[t-1] + S$rCS[t] - S$jCP[t], 0)
    # Rejection flux: excess light energy not quenched by carbon fixation
...
run_coral_wC <- function(time, env, pars, NPQ="inhibited") {
  require(dplyr)
  # Define parallel complementary synthesizing unit function
  synth <- function(x, y, m) 1/((1/m)+(1/x)+(1/y)-(1/(x+y)))
  #synth <- function(x, y, m) (m^-2 + x^-2 + y^-2)^(-1/2)
  ...
  for (t in 2:length(time)) {
    ...
    # Production flux (photosynthetic carbon fixation)
    S$jCP[t] <- synth(S$jL[t] * pars$nLC, (5*H$jCw[t-1] + H$jCO2[t] + H$rCH[t])*H$H[t-1]/S$S[t-1] + S$rCS[t], pars$jCPm) / S$cROS[t-1]
