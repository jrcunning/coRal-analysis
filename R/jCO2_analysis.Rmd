...
sapply(c("../R/def_pars.R", "../R/init_env.R", "../R/run_coral.R", "../R/plot_results.R"), source, .GlobalEnv)
```
...
```{r run_models, cache=T, message=F, warning=F, include=F}
##### VARY NITROGEN
...
### Now remove feeding
```{r run_models_nofeeding, cache=T, message=F, warning=F}
##### VARY NITROGEN
...
### Now make jPNQ inhibited
```{r run_models_nofeeding_jNPQinhibited, message=F, warning=F, cache=T}
##### VARY NITROGEN
# RUN USING WASTED CARBON AND NO JCO2A OR JCO2P
time <- seq(1,1230,1)
defpars <- def_pars()
...
### Trying different stress parameter sets
```{r par_tests, cache=T, warning=F, message=F}
##### VARY LIGHT
# RUN USING WASTED CARBON AND NO JCO2A OR JCO2P
time <- seq(1,1230,0.5)
