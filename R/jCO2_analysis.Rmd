...
run_coral_wC <- function(time, env, pars, NPQ="inhibited", jDOC=0) {
  require(dplyr)
  # Define parallel complementary synthesizing unit function
  synth <- function(x, y, m) 1/((1/m)+(1/x)+(1/y)-(1/(x+y)))
  #synth <- function(x, y, m) (m^-2 + x^-2 + y^-2)^(-1/2)
  ...
  for (t in 2:length(time)) {
    ...
    # Production flux (host biomass formation)
    H$jHG[t] <- synth(S$rhoC[t]*S$S[t-1]/H$H[t-1] + H$jX[t] + jDOC, (H$jN[t] + pars$nNX*H$jX[t] + H$rNH[t]) / pars$nNH, pars$jHGm)
    # Rejection flux: nitrogen (surplus nitrogen shared with the symbiont)
    H$rhoN[t] <- max(H$jN[t] + pars$nNX * H$jX[t] + H$rNH[t] - pars$nNH * H$jHG[t], 0)
...
#### possibility of DOC uptake?
```{r incr_L_DOC, cache=T, message=F, warning=F, include=F, eval=F}
# Set up simulation
env <- init_env(time=time, L=c(2,60,0), N=c(1e-7,1e-7,0), X=c(0,0,0))
# Run new model (sets jCO2a and jCO2p to zero; CO2 is delivered by jCw)
run <- run_coral_wC(time=time, env=env, jDOC=0.005,
                 pars=replace(defpars, 
                              c("kNPQ", "kROS", "k", "b", "sigmaCS", "sigmaCH", "jCO2a", "jCO2p"), 
                              c(60,       60,    1,   5,      0.9,         0.9, 0, 0)),
                 NPQ="fixed2")
...
run_steady_states <- function(time, pars, at, outfile=NULL, runtime=F, food=0, jDOC=0) {
  #
  ...
  steady_states <- foreach(i=1:nrow(at), .combine='rbind', .packages='dplyr') %dopar% {
    ...
    run <- run_coral_wC(time=time, env=env, pars=pars, jDOC=jDOC)
...
## When system starts at 2 µM DIN and increases to 4 µM DIN, outcome is negative growth.
...
## When system starts at 1 µM DIN and increases to 4 µM DIN, outcome is positive growth.
