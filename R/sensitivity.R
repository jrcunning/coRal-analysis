...
sens.plot <- function(response, pars, cols, lines="response") {
  ...
  # Add legends
  legend("topright", legend=pars, lty=1, col=cols, bty="n")
  if (lines=="response") {
    legend("topleft", legend=response, lty=1:nlevels(factor(response)), bty="n")
  } else {
    legend("topleft", legend=levels(dat[[1]]$env), lty=1:nlevels(dat[[1]]$env), bty="n")
  }
}
