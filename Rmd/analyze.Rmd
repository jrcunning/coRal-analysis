...
### Light amplification & self-shading
...
# Run to steady state and plot results
ssdat <- run_steady_states(pars=replace(defpars, c("initS", "kCO2"), c(1, 10)), at=at, food=5e-6, runtime=T)
plot_steady_states(ssdat)
```
With default parameters, steady state values of specific growth are higher than usually observed in corals (skeletal growth), but patterns across environments are consistent with literature. 
...
env <- init_env(time=time, L=c(20,20,2), N=c(1e-7,4e-6,2), X=c(0,0,0))

# Set parameters
defpars <- def_pars()  # Get default parameters

# Run simulation
run <- run_coral(time=time, env=env, pars=replace(defpars, c("initS", "kCO2"), c(0.25, 10)))#
plot_run(run)
```
